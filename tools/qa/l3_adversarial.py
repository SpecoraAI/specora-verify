...
def signature_suite():
    ...
    # SIG-00 baseline
    record("SIG-00", "signature", "valid sig, correct key", PASS_OK, accepted(h, good_sig, pk))
    ...
    record(
        "SIG-01",
        "signature",
        "1-field tamper (pass->fail flipped)",
        MUST_FAIL,
    ...
    record(
        "SIG-02",
        "signature",
        "valid sig verified vs WRONG pubkey",
        MUST_FAIL,
    ...
    record("SIG-06", "signature", "empty signature string", MUST_FAIL, accepted(h, "", pk))

    # SIG-07 uppercase hex hash (code requires lowercase)
    record(
        "SIG-07", "signature", "uppercase hex hash", MUST_FAIL, accepted(h.upper(), good_sig, pk)
    )
    ...
    record(
        "SIG-09",
        "signature",
        "valid sig over H1 presented with H2",
        MUST_FAIL,
    ...
    record(
        "SIG-11",
        "signature",
        "attacker-signed hash vs pinned real key",
        MUST_FAIL,
...
def canon_suite():
    ...
    record("CANON-03", "canonical", "reordered keys -> same hash (determinism)", PASS_OK, same)

    # CANON-04 duplicate-key smuggling: two different raw texts, one parsed dict
    raw1 = '{"amount": 100, "amount": 1}'  # last-wins in json.loads
    raw2 = '{"amount": 1}'
    same_hash = sha256_hex(canonical_json_bytes(json.loads(raw1))) == sha256_hex(
        canonical_json_bytes(json.loads(raw2))
    )
    record(
        "CANON-04",
        "canonical",
...
def mint_cert(
    ...
    *,
    ...
    cert["signature"] = base64.b64encode(issuer_sk.sign(canonical_json_bytes(unsigned))).decode()
    return cert
...
def aid_accepted(cert, issuer_hex, now):
    try:
        return validate_agent_identity_certificate(
            cert, issuer_public_key_hex=issuer_hex, now=now
        ).valid
    except Exception:
        return False
...
def aid_suite():
    ...
    c = mint_cert(ISSUER, issued_at="2026-05-08T12:00:00Z", not_after="2026-06-07T12:00:00Z")
    record(
        "AID-00", "agent-id", "valid cert, pinned issuer", PASS_OK, aid_accepted(c, iss_hex, now)
    )
    ...
    record(
        "AID-01",
        "agent-id",
        "tamper principal.public_key post-sign",
        MUST_FAIL,
    ...
    record(
        "AID-02",
        "agent-id",
        "tamper subject.org_id post-sign",
        MUST_FAIL,
        aid_accepted(t, iss_hex, now),
    )

    # AID-03 expired
    exp = mint_cert(ISSUER, issued_at="2025-01-01T00:00:00Z", not_after="2025-01-02T00:00:00Z")
    record(
        "AID-03",
        "agent-id",
        "expired cert (now >= not_after)",
        MUST_FAIL,
        aid_accepted(exp, iss_hex, now),
    )

    # AID-04 not yet valid
    nyv = mint_cert(ISSUER, issued_at="2027-01-01T00:00:00Z", not_after="2027-02-01T00:00:00Z")
    record(
        "AID-04",
        "agent-id",
        "not-yet-valid (now < issued_at)",
        MUST_FAIL,
    ...
    record(
        "AID-05",
        "agent-id",
        "valid cert vs WRONG pinned issuer key",
        MUST_FAIL,
    ...
    record(
        "AID-06",
        "agent-id",
        "unsupported cert format string",
        MUST_FAIL,
        aid_accepted(badfmt, iss_hex, now),
    )

    # AID-07 missing principal block (re-sign so signature is otherwise valid)
    noprinc = mint_cert(ISSUER, issued_at="2026-05-08T12:00:00Z", not_after="2026-06-07T12:00:00Z")
    del noprinc["principal"]
    unsigned = {k: v for k, v in noprinc.items() if k != "signature"}
    noprinc["signature"] = base64.b64encode(ISSUER.sign(canonical_json_bytes(unsigned))).decode()
    record(
        "AID-07",
        "agent-id",
        "principal block absent (validly signed)",
        MUST_FAIL,
    ...
    record(
        "AID-08",
        "agent-id",
        "principal.public_key not 64 hex chars",
        MUST_FAIL,
    ...
    forged = mint_cert(
        ...
        issued_at="2026-05-08T12:00:00Z",
        not_after="2026-06-07T12:00:00Z",
        fingerprint=public_key_fingerprint(iss_hex),
    )  # claims real fp
    record(
        "AID-09",
        "agent-id",
        "attacker self-sign + spoofed fingerprint",
        MUST_FAIL,
        aid_accepted(forged, iss_hex, now),
    )

    # AID-10 boundary: now == not_after  -> expired (code: now >= not_after)
    bound = mint_cert(ISSUER, issued_at="2026-05-08T12:00:00Z", not_after="2026-05-15T12:00:00Z")
    record(
        "AID-10",
        "agent-id",
    ...
    bound2 = mint_cert(ISSUER, issued_at="2026-05-15T12:00:00Z", not_after="2026-06-01T12:00:00Z")
    record(
        "AID-11",
        "agent-id",
        "boundary now == issued_at (valid)",
        PASS_OK,
        aid_accepted(bound2, iss_hex, now),
    )

    # AID-12 REVOKED-but-cryptographically-valid cert (THM-AID-REV gap)
    revoked = mint_cert(ISSUER, issued_at="2026-05-08T12:00:00Z", not_after="2026-06-07T12:00:00Z")
    revoked_accepted = aid_accepted(revoked, iss_hex, now)
    record(
        "AID-12",
        "agent-id",
        "revoked cert still validates (no revocation input)",
        FLAG,
        revoked_accepted,
    ...
    naive = mint_cert(
        ISSUER, issued_at="2026-05-08T12:00:00", not_after="2026-06-07T12:00:00"
    )  # no 'Z'
    naive_accepted = aid_accepted(naive, iss_hex, now)
    record(
        "AID-13",
        "agent-id",
        "tz-naive validity window (local-tz coercion)",
        FLAG,
        naive_accepted,
    ...
    record(
        "AID-14",
        "agent-id",
        "uppercase principal.public_key accepted",
        FLAG,
        upper_accepted,
...
def bundle_suite():
    iss_hex = pub_hex(ISSUER)
    now = datetime(2026, 5, 15, 12, 0, 0, tzinfo=UTC)
    cert = mint_cert(ISSUER, issued_at="2026-05-08T12:00:00Z", not_after="2026-06-07T12:00:00Z")

    def bundle_valid(bundle, issuer_hex):
        try:
            return validate_bundle_v1_1(bundle, issuer_public_key_hex=issuer_hex, now=now).valid
        except Exception:
            return False
    ...
    record(
        "BUNDLE-01",
        "bundle",
        "valid bundle w/ identity + issuer",
        PASS_OK,
    ...
    record(
        "BUNDLE-02",
        "bundle",
        "tampered agent_identity in record",
        MUST_FAIL,
    ...
    record(
        "BUNDLE-03",
        "bundle",
        "identity present, issuer key = None",
        MUST_FAIL,
        bundle_valid(b, None),
    )
    ...
    record(
        "BUNDLE-05",
        "bundle",
        "empty records list (vacuous PASS)",
        FLAG,
        empty_accepted,
...
def main():
    ...
    graded = total - flagn
    print(
        f"Graded cases: {graded}   passed: {graded - defects}   DEFECTS: {defects}   flags: {flagn}"
    )
    ...
    print(
        "VERDICT:",
        "DEFECTS FOUND" if crit else "No forged-PASS defects. See trust flags for disclosure gaps.",
    )
    print("=" * width)
