...
> Canonical hosted mirror of [`docs/wire-spec-v1.0.md`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/wire-spec-v1.0.md). Authoritative source lives in the public git repo.
...
> This document describes a contract that already ships. It does not propose changes. If the spec and the reference implementation disagree, the reference implementation wins and the spec is amended to match — see [§10 change log](#10-change-log) and [versioning-policy.md](https://github.com/SpecoraAI/specora-verify/blob/main/docs/versioning-policy.md).
...
- Not versioning guidance for customer integrations. That lives in [versioning-policy.md](https://github.com/SpecoraAI/specora-verify/blob/main/docs/versioning-policy.md).
...
This spec exists because Specora is structurally the auditor and cannot also be the producer. The trust-bootstrap model (Big 4 auditors, NRSRO credit raters, Web PKI, EU AI Act Notified Bodies — see [trust-model.md](https://github.com/SpecoraAI/specora-verify/blob/main/docs/trust-model.md)) requires a document the producer **cannot** rewrite. That is why the canonical copy lives in a public git repository with signed commits and PR review, not behind a subdomain Specora can silently mutate.

---

## 2. Data model

A Specora evidence bundle is a set of files on disk (or equivalent in-memory objects). Each file belongs to exactly one of seven canonical payload types. All seven are already present as golden vectors in [`vectors/`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/) and are the normative reference for the field definitions below.

### 2.1 Summary table

| # | Payload type | `spec_id` field | Schema file | Vector location |
|---|---|---|---|---|
| 1 | Attestation Manifest | `attestation-manifest` | [`schemas/attestation-manifest-v1.0.json`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/schemas/attestation-manifest-v1.0.json) | [`vectors/manifest/attestation-manifest-1.0.0.canonical.json`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/manifest/attestation-manifest-1.0.0.canonical.json) |
| 2 | Proof Manifest | `proof-manifest` | [`schemas/proof-manifest-v1.0.json`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/schemas/proof-manifest-v1.0.json) | [`vectors/manifest/proof-manifest-1.0.0.canonical.json`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/manifest/proof-manifest-1.0.0.canonical.json) |
| 3 | Anchor Payload | `anchor-payload` | [`schemas/anchor-payload-v1.0.json`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/schemas/anchor-payload-v1.0.json) | [`vectors/anchor/anchor-payload-1.0.0.canonical.json`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/anchor/anchor-payload-1.0.0.canonical.json) |
| 4 | Anchor Receipt | `anchor-receipt` | [`schemas/anchor-receipt-v1.0.json`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/schemas/anchor-receipt-v1.0.json) | [`vectors/anchor-receipts/anchor-receipt-1.0.0.canonical.json`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/anchor-receipts/anchor-receipt-1.0.0.canonical.json) |
| 5 | Certification Attestation | `certification-attestation` | [`schemas/certification-attestation-v1.0.json`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/schemas/certification-attestation-v1.0.json) | [`vectors/certification/certification-attestation-1.0.0.canonical.json`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/certification/certification-attestation-1.0.0.canonical.json) |
| 6 | STP Certification Attestation | `stp-certification-attestation` | [`schemas/stp-certification-attestation-v1.0.json`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/schemas/stp-certification-attestation-v1.0.json) | [`vectors/stp-certification/compatible/stp-certification-attestation-1.0.0.canonical.json`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/stp-certification/compatible/stp-certification-attestation-1.0.0.canonical.json) |
| 7 | Signed Artifact Envelope | `governance-attestation` (example) | [`schemas/signed-artifact-envelope-v1.0.json`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/schemas/signed-artifact-envelope-v1.0.json) | [`vectors/signature/signed-artifact-001/`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/signature/signed-artifact-001/) |
...
See [`schemas/attestation-manifest-v1.0.json`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/schemas/attestation-manifest-v1.0.json) for the normative field definitions and [`vectors/manifest/attestation-manifest-1.0.0.canonical.json`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/manifest/attestation-manifest-1.0.0.canonical.json) for the golden example.
...
The on-disk layout that pairs any canonical payload with its detached Ed25519 signature and verification metadata. See [`vectors/signature/signed-artifact-001/`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/vectors/signature/signed-artifact-001/) for the reference layout.
...
- **Verifier side:** [`specora_verify/canonical.py`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/canonical.py) in this repo.
...
5. Report a structured `SignatureVerificationResult` with explicit `valid` bool, `errors` list, and key metadata. The reference struct is [`specora_verify/signature.py::SignatureVerificationResult`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/signature.py).
...
The reference implementation is [`specora_verify/hash.py::sha256_hex`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/hash.py).
...
See [`versioning-policy.md`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/versioning-policy.md) for the normative version-management rules. Summary:
...
- **[`specora_verify/canonical.py`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/canonical.py)** — verifier-side reference implementation of §3.
- **[`specora_verify/signature.py`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/signature.py)** — verifier-side reference implementation of §4.
- **[`specora_verify/hash.py`](https://github.com/SpecoraAI/specora-verify/blob/main/specora_verify/hash.py)** — verifier-side reference implementation of §5.
...
- **[`docs/trust-model.md`](https://github.com/SpecoraAI/specora-verify/blob/main/docs/trust-model.md)** — audit-doctrine framing that motivates this spec's existence and structural shape.
